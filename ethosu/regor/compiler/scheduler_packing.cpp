    ...
    const SchedulerOperation *nextOp, const int prevOpKey) const
{
    ...
    // Can only fuse activations with unit scaling
    if ( !IsElementwise(nextOp->Type()) && !(nextConnOfm->quantization.IsUnitScale() && nextConnIfm->quantization.IsUnitScale()) )
    {
        return 0;
    }
...
std::vector<std::unique_ptr<SchedulerOperation>> SchedulerPacking::DecomposeSchedulerOperation(std::unique_ptr<SchedulerOperation> op)
{
    ...
    switch ( op->Type() )
    {
        ...
        case OpType::MemoryCopy:
            result = DecomposeMemoryCopy(_ctx, std::move(op));
            break;
        default:
